...
namespace Reminder.Domain
{
	public class ReminderDomain : IDisposable
	{
		...
		public ReminderDomain(
			...
			IReminderReceiver receiver,
			IReminderSender sender,
			TimeSpan awaitingRemindersCheckPeriod,
		...
		{
			var readyItems = _storage
				.GetList(new[] { ReminderItemStatus.Awaiting })
		...
		{
			var readyItems = _storage
				.GetList(new[] { ReminderItemStatus.ReadyToSend });
			...
			foreach (var readyItem in readyItems)
			{
				...
				try
				{
					...
					SendingSucceeded?.Invoke(
						this,
						new SendingSucceededEventArgs(sendingModel));
				}
				catch(Exception exception)
				{
					...
					SendingFailed?.Invoke(
						this,
						new SendingFailedEventArgs(sendingModel, exception));
				}
